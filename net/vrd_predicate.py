...
#############################################################	
	...
	def build_rd_network(self):
		...
		sub_obj_p = slim.fully_connected(self.sub_obj_p, cfg.VTR.VG_R, 
										 activation_fn=tf.nn.relu, scope='RD_l1', reuse = False)
		...
		ful_final_all_1 = slim.fully_connected(full_fc1 , cfg.VTR.VG_R, 
										 activation_fn=tf.nn.relu, scope='RD_full_all', reuse = False)
		rela_score_full = slim.fully_connected(ful_final_all_1, self.num_predicates,  
										 activation_fn=None, scope='RD_final_all', reuse = False)

		self.predictions['rela_pred_all'] = tf.concat([tf.nn.sigmoid(rela_score_full)], axis = 1)
		self.layers['rela_score_f'] = rela_score_full			

		self.layers['rd_loss'] = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits( labels=rela_label, logits=rela_score_full))
#( - tf.reduce_sum( rela_label * tf.log(tf.clip_by_value( tf.nn.sigmoid(rela_score_full), 1e-8, 1.0)))  - tf.reduce_sum( ( 1-rela_label) * tf.log(tf.clip_by_value(1 - #tf.nn.sigmoid(rela_score_full), 1e-8, 1.0))) )
	...
	def add_rd_loss(self):
		...
		f = tf.reduce_sum(rela_score, axis = 1)	
		self.predictions['rela_max_prob'] = rela_max_prob 

	def train_predicate(self, sess, roidb_use, RD_train, N, o):
	...
	def test_predicate(self, sess, roidb_use, o):
		im, im_scale, w, h = im_preprocess(roidb_use['image'])
		...
		pred_rela_all = predictions['rela_pred_all'][:,:]
